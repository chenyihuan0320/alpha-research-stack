...
AkShare 的 A 股/港股日线接口使用 Eastmoney 上游端点。provider probe 默认对 Eastmoney 使用直连模式：调用期间临时绕过 `HTTP_PROXY` / `HTTPS_PROXY` 等代理变量，并在结束后恢复原环境。这个设置只服务于 provider validation，不改变其他 provider 的代理策略。
