...
def _run_case(
    *,
    ...
    try:
        with _context_for_mode(proxy_mode):
            frame = call()
        shape, columns = _summarize_frame(frame)
        return DiagnosticRecord(market, ticker, interface, proxy_mode, "success", shape, columns, "")
    except Exception as exc:  # provider/network dependent
        return DiagnosticRecord(market, ticker, interface, proxy_mode, "failed", "-", [], str(exc))
...
def _recommend_proxy_mode(records: list[DiagnosticRecord]) -> str:
    success_counts = {
        mode: sum(1 for record in records if record.proxy_mode == mode and record.status == "success")
        for mode in PROXY_MODES
    }
...
def write_report(records: list[DiagnosticRecord], metadata: dict[str, str | bool | list[str]]) -> None:
    ...
    for record in records:
        ...
        lines.append(
            f"| {record.market} | {record.ticker} | {record.interface} | {record.proxy_mode} | "
...
def main() -> int:
    try:
        ...
        records = [
            DiagnosticRecord("-", "-", "-", mode, "failed", "-", [], str(exc))
            for mode in PROXY_MODES
        ]
