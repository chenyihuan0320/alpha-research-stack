...
- 估值成功项：`stock_zh_valuation_baidu` 对 `600519.SH`、`000001.SZ`、`300750.SZ` 返回 A 股估值样本；当前只覆盖 `market_cap`、`pe`、`pb`，且必须检查 indicator as-of 日期一致性。
...
- 将 A 股估值合并改为 latest merged snapshot，记录每个 indicator 的 as-of 日期；如果 `market_cap`、`pe`、`pb` 日期不一致，写入 `asof_mismatch`。
...
- A 股估值质量标记：如果 `market_cap`、`pe`、`pb` 的 provider 日期不一致，必须标记 `asof_mismatch`；缺失的估值字段标记 `partial_coverage`。
