...
Reuse Gate 是项目硬规则：准备自建候选发现、回测、报告推送或 Agent 研究前，必须先验证对应开源项目并记录结论。候选发现优先验证 AlphaSift，轻量验证优先验证 vectorbt，因子/模型研究优先验证 Qlib，报告推送优先验证 daily_stock_analysis，深度研究优先验证 TradingAgents / FinRobot。
...
已完成 AkShare / Tushare 数据源验证和 `data_quality_gate`。当前进入 Reuse Gate 与 AlphaSift / vectorbt 复用验证阶段；不自建候选发现和回测，除非复用验证失败并在 `docs/reuse_decisions.md` 记录原因。
